...
#1 {main}', $stack);
    ...
    {
        $rawException = <<<EOL
GuzzleHttp\Exception\ServerException: Server error: POST resulted in a `500 Internal Server Error` in /tmp/failed-jobs-monitor/vendor/yay-innovations/test/src/Class.php:180
Stack trace:
#0 /tmp/failed-jobs-monitor/vendor/test/test/src/Class.php(185): Illuminate\Foundation\Bootstrap\HandleExceptions->handleError(8, 'Undefined index...', '/tmp...', 185, Array)
#1 {main}
EOL;
        ...
        $this->assertEquals('GuzzleHttp\Exception\ServerException: Server error: POST resulted in a `500 Internal Server Error`', $exception->getError());
        $this->assertEquals('vendor/yay-innovations/test/src/Class.php:180', $exception->getLocation());
